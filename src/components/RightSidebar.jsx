...
export default function RightSidebar({ articles: articleList, timelineEvents }) {
  ...
  // Sort newest-first before slicing — articleList is in id order, so without this
  // the "最新" (Latest) widget shows the oldest articles.
  const latestArticles = [...articleList]
    .sort((a, b) => new Date(b.date) - new Date(a.date))
    .slice(0, 3);

  return (
    <aside className="w-56 hidden lg:block border-l border-hair dark:border-[#2E2B23] p-5 fixed top-0 right-0 h-screen z-30 overflow-y-auto bg-white dark:bg-[#1C1A14]">
